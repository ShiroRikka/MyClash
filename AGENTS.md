...
- **地区检测**：通过国旗 emoji (flagMap) 匹配代理名，动态生成代理组（HK/TW/JP/US/SG/KR/CN/GB/DE/FR/AU/CA）
- **区域标签检测**：通过区域文字标签（亚洲/北美洲/欧洲等）动态生成代理组
- **倍率分组**：按【Nx】标签分组：高倍率专线(≥10x)、普通倍率(2x-9x)、低倍率(<2x)
- **质量分级**：按 A/B/C 前缀分类，创建质量回退链（A级→B级→C级）
- **直连识别**：识别节点名中包含"直连"的节点，创建独立分组
- **带宽分级**：`(\d+(?:\.\d+)?)\s*MB/s` 正则提取，按数值降序分组 (1-5MB/s各一组，>=6MB/s为6+MB/s组)
- **规则集**：从 Loyalsoldier/clash-rules 加载，默认结转 `MATCH,节点选择`
- **代理组**：节点选择 → 质量/倍率/直连 → 地区/区域 → 解锁/带宽 → 全局策略 → 广告/应用/全球

## 注意

1. 每次更改都需要版本更新：根据本次修改的大小，修改顶部 `// vX.Y` 注释
2. 中文代理组名和注释不翻译保留
3. flagMap 已扩展覆盖常见订阅源，4 个地区→ 12 个地区： 支持 HK/TW/JP/US/SG/KR/CN/GB/DE/FR/AU/CA
